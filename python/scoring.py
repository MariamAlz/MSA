...
from msa import MSA
from substitution_matrix import SubstitutionMatrix, PAM250
from tool import StrikeEx
...
def calc_sum_pair(sequences):
    ...
    for i in range(t):
        ...
        for j in range(i + 1, t):
            B = sequences[j]
            for idx in range(k):
                if A[idx] == B[idx] and A[idx] != '-':
                    score += 1
    return score

def calc_sum_pair_include_gap_penalty(sequences):
    t = len(sequences)
    k = len(sequences[0])
    score = 0
    for i in range(t):
        ...
        for j in range(i + 1, t):
            ...
            for idx in range(k):
                ...
                elif A[idx] != B[idx] and A[idx] != '-' and B[idx] != '-':
                    score -= 1
                # subtract 1 for gap
                elif A[idx] == '-' or B[idx] == '-':
                    score -= 1
